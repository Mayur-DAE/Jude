...
export class VerificationComponent implements OnInit {
  ...
  verifyOtp() {
    ...
    this.service.verifyOTP(this.MobileNumber, otp).subscribe((data) => {
      ...
      if (status == 'Success') {
        ...
        showDangerToast('The OTP entered is incorrect.');
      }
    });
  }
  ...
  validateForm(): boolean {
    ...
    if (!this.MobileNumber) {
      // console.log("ERROR: this.MobileNumber is undefined or null");
      this.mobilenumber_errorMsg = 'Please Enter mobile number';
      return false;
    }
    const trimmedMobileNumber = this.MobileNumber.trim();
    // console.log(" MobileNumber after trim:", trimmedMobileNumber);
    if (trimmedMobileNumber.length === 0) {
      ...
      isFormValidate = false;
      this.mobilenumber_errorMsg = 'Invalid contact no. format.';
    } 
